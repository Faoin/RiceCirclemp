  ...
  <div class="commoditySum" :class="{commoditySumStatus: commoditySumShopp}" @click="commodityToggleShowList">
    ...
    <div class="commodityD">
      <p class="commodityDM" :class="{commodityDMStatus: commoditySumShopp }">
        {{ commoditySumShopp === 0 ? '未选购商品' :  commodityRealPrice + '元' }}
      </p>
      <p class="commodityDD">
        不足十元需要加一元配送费
      </p>
      ...
      fly.get('https://fquano.cn/system/food/getFood4ShopList?id=' + index)
        .then(function (response) {
          if (response.status === 200) {
            ...
            _this.commodityName = response.data.data.commodityName
            // 菜品分类
            // let commondityVal = response.data.data
            // 菜品信息
            let commondityDetailVal = response.data.data.commodityList
            _this.commodityList = commondityDetailVal
            console.log('url:' + commondityDetailVal.commodityMenu)
            console.log(response.data.data.commodityName)
